...
public class LoggingDelegatingResultSet implements ResultSet {
	private final ResultSet wrappedResultSet;
	private final Logger logger;

	public LoggingDelegatingResultSet(final ResultSet wrappedResultSet, Logger logger) {
	...
	public boolean next() throws SQLException {
		boolean result = wrappedResultSet.next();
		if (result) {
			if (wrappedResultSet.isFirst()) {
				String columnNames = getColumnNames();
				logger.trace(columnNames);
			}
			String columnValues = getColumnValues();
			logger.trace(columnValues);
		}
		return result;
	}
	...
	private String getColumnValues() throws SQLException {
		...
		for (int i = 1; i <= columnCount; i++) {
			...
			String value = StringUtils.defaultString(wrappedResultSet.getString(i));
			String columnClassName = metaData.getColumnClassName(i);
			if (isDateType(columnClassName)) {
				Timestamp timestamp = wrappedResultSet.getTimestamp(i);
				if (timestamp != null) {
					value = String.valueOf(timestamp.getTime());
				}
			}
			columnValues.append("'" + columnClassName + ":" + value + "'");
		}

		return columnValues.toString();
	}

	private boolean isDateType(String columnClassName) {
		boolean result = false;
		result |= Date.class.getName().equals(columnClassName);
		result |= Time.class.getName().equals(columnClassName);
		result |= Timestamp.class.getName().equals(columnClassName);
		return result;
	}
	...
	public String getString(int columnIndex) throws SQLException {
		return StringUtils.defaultString(wrappedResultSet.getString(columnIndex));
	}
